...
CiaoTool Solana 多对一转账是一款批量资产归集工具，可将多个钱包中的 SOL 或支持的 SPL 代币，集中转入一个指定收款地址。

系统根据钱包数量自动生成并分组执行转账任务，省去逐个切换钱包、填写地址和签名发送的重复操作，适用于多钱包资产回收与统一管理。
...
* **批量高效：** 一次配置多个转出钱包，系统自动生成并执行归集任务
* **灵活设置：** 支持批量导入，并可分别设置各钱包的归集金额
* **发送方式可选：** 支持 RPC 统一广播或 Jito 打包发送
* **过程可核对：** 提交前可确认转出钱包数量、归集总额及预估费用
