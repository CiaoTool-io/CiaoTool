...
**CiaoTool Bonk 卖出并多地址捆绑买入**是一款面向 Bonk.fun 的组合交易工具。用户可设置卖出钱包、卖出数量、买入钱包及各钱包的买入金额，系统自动构建“先卖出、后买入”的交易序列，并通过 Bundle 提交。

正常执行时，卖出与多笔买入交易将在同一区块内按预设顺序连续完成，减少交易间隙及频繁切换钱包的操作成本，提升组合交易的执行一致性。
...
* **买卖组合执行：** 将指定钱包卖出与多个钱包买入组成完整交易序列
* **同区块连续执行：** Bundle 上链后，包内交易按预设顺序完成
* **减少交易间隙：** 降低卖出与买入之间被其他交易插入的风险
* **多钱包配置：** 支持分别设置买入地址及对应金额
* **自动化处理：** 自动完成交易构建、签名组织及 Bundle 提交
