---
description: >-
  专为 Pump.fun 和 PumpSwap 打造的高效交易换仓工具，通过同区块聚合执行卖出与多钱包买入，模拟真实盘口交易形式让换仓行为更自然。在有效降低
  MEV 夹击风险与资金损耗的同时，安全优化链上资产轨迹。
---

# Pump - 卖出并多地址捆绑买入指南

{% hint style="info" %}
**CiaoTool Pump 多地址捆绑卖出**现已全面支持官方 **SOL** 和 **USDC** 全部的价值代币
{% endhint %}

## 功能介绍

**CiaoTool Pump 卖出并多地址捆绑买入**是一款面向 Pump.fun 与 PumpSwap 的组合交易工具。用户可设置卖出钱包、卖出数量、买入钱包及各钱包的买入金额，系统自动构建“先卖出、后买入”的交易序列，并通过 Bundle 提交。
