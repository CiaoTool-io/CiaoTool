...
{% hint style="info" %}
**CiaoTool Pump 提升交易人数**现已全面支持官方 **SOL** 和 **USDC** 全部的价值代币
{% endhint %}

## 功能介绍

**CiaoTool Pump 提升交易人数**是一款面向 Pump.fun 与 PumpSwap 的多地址自动交易工具，可批量生成新钱包并执行指定代币的买入操作。
...
* **全流程自动化：** 一键完成钱包生成、资金分配、买入、归集及账户清理
* **批量地址交互：** 支持多个新钱包分别完成链上买入
* **降低账户成本：** 关闭余额为零的临时代币账户并回收可退租金
* **统一资产归集：** 自动将代币与剩余资金转入指定钱包
* **过程可追踪：** 展示各地址的交易状态、买入结果和归集记录
* **本地签名：** 临时钱包及主钱包私钥仅在浏览器本地用于交易授权
