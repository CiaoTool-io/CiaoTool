...
# Solana - Boost Holders Guide

{% hint style="info" %}
**CiaoTool Raydium Boost Holders** now fully supports all liquidity pool types, including **AMM V4, CLMM, and CPMM**.
{% endhint %}

## Overview

**CiaoTool Solana Holder Booster** is a multi-address token distribution tool for Raydium. It generates new wallets in batches, distributes trading funds, and uses each wallet to purchase the selected token. The purchased tokens remain in their respective wallets, creating active token-holding accounts.

Start your Boost Holders on Raydium with CiaoTool now:

{% embed url="https://ciaotool.io/en/raydium/new-address-holders" %}
