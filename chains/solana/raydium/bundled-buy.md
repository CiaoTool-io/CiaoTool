...
{% hint style="info" %}
**CiaoTool Raydium 多地址捆绑买入**现已全面支持 **AMM V4、CLMM、CPMM** 全部的流动性池类型，请先切换到指定池子功能页面进行多地址捆绑买入操作，满足不同场景下的快捷买入服务。
{% endhint %}
...
{% embed url="https://ciaotool.io/zh-Hans/raydium/multi-address-buy" %}
...
**Raydium 多地址捆绑买入**导入钱包没有数量限制。服务费均由每组第一个地址支付，Jito 小费由导入的最后一个地址支付，调整导入的第一个和最后一个地址中有足够余额。
{% endhint %}
